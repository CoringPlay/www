...
class game extends db
{
     ...
     * @return array|null
     */
    function getIduserInf($userdubl,$id)
    {
        $this->Query("SELECT * FROM lottery_users WHERE lot_id = '{$id}' AND user_id = '{$userdubl}'");
      return $this->FetchArray();
    }
	/**
     * @param $room
 ...
 /**
     ...
     * @param $sum
     */
    function setUserReiting($user_id, $reiting)
    {
        $this->Query("UPDATE users_conf SET reiting = reiting + '{$reiting}' WHERE user_id = '{$user_id}'");
    }
      /**
     * @param $id
     * @param $sum
    ...
    {
        $this->Query("UPDATE users SET bet = bet + '{$igr_ko}' WHERE id = '{$id}'");
    }
    /**
     * @param $id
     * @return array|null
